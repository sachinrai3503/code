...
// https://www.geeksforgeeks.org/count-possible-decodings-of-a-given-digit-sequence-in-on-time-and-constant-auxiliary-space/
    ...
    for(;i>=0;i--){
        ...
        if(s[i]!='0'){
            if(i==length-1) count = 1;
                ...
                count = last_count;
                int t_num = to_digit(s[i],s[i+1]);
                if(t_num>=1 && t_num<=26){
                    if(i==length-2) count+=1;
                    else count+=last_to_last_count;
                }
            }
        }
