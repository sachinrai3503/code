...
Explanation: There are 12 different happy string of length 3 ["aba", "abc",
        "aca", "acb", "bab", "bac", "bca", "bcb", "cab", "cac", "cba", "cbc"]. 
...
class Solution:
    def getHappyString(self, n: int, k: int) -> str:
        max_permuatation_count = 3*(2**(n-1))
        if k<=0 or k>max_permuatation_count: return ''
        op = list()
        adj_char_map = {'~':['a','b','c'],'a':['b','c'],'b':['a','c'],'c':['a','b']}
        prev = '~'
        length = n
        while k>1:
            t_count = 2**(length-1)
            quo, rem = k//t_count, k%t_count
            quo+=(0 if rem==0 else 1)
            t_char = adj_char_map.get(prev)[quo-1]
            op.append(t_char)
            k-=((quo-1)*t_count)
            prev = t_char
            length = length -1
        while len(op)<n:
            op.append(adj_char_map.get(prev)[0])
            prev = op[-1]
        return ''.join(op)