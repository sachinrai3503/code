...
Explanation: The largest subset with at most 5 0's and 3 1's is 
             ...
             {"10", "1", "0"}. {"111001"} is an invalid subset because it
             contains 4 1's, greater than the maximum of 3.
...
class Solution:
    def findMaxForm(self, strs: list[str], m: int, n: int) -> int:
        ...
        for i in range(len(strs)-1, -1, -1):
            _str = strs[i]
            count_0, count_1 = get_0_1_count(_str)
            for i in range(m, count_0-1, -1):
                for j in range(n, count_1-1, -1):
                    op[i][j] = get_max(op[i][j], op[i-count_0][j-count_1] + 1)
            # for row in op:
            #     print(row)
            # print('===========================================')
        return op[m][n]
