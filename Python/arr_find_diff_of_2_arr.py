...
class Solution:
    def findDifference(self, nums1: List[int], nums2: List[int]) -> List[List[int]]:
        ...
        max_range = 2001
        min_ele = -1000
        map1 = [None for i in range(max_range)]
        map2 = [None for i in range(max_range)]
        for num in nums1:
            map1[num-min_ele] = num
        for num in nums2:
            map2[num-min_ele] = num
        for i in range(max_range):
            if map1[i] is not None and map2[i] is not None: continue
            if map1[i] is not None: op[0].append(map1[i])
            elif map2[i] is not None: op[1].append(map2[i])
        return op