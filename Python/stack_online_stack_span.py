...
Also, if the prices of the stock in the last four days is [7,34,1,2] and the price of the stock today is 8, then the span of
 today is 3 because starting from today, the price of the stock was less than or equal 8 for 3 consecutive days.
...
class StockSpanner:
    ...
    def next(self, price: int) -> int:
        ...
        self.count+=1
        i = self.count-1
        while self.stck and self.items[self.stck[-1]]<=price:
            self.stck.pop()
        op = self.count if not self.stck else i-self.stck[-1]
        self.stck.append(i)
        return op
