...
class Node:
    def __init__(self, gas, dest, cur):
        self.gas = gas
        self.dest = dest
        self.cur = cur

    def __str__(self):
        return ':'.join([str(self.gas),str(self.dest),str(self.cur)])
        
class Solution:

    # Below queue sol won't work for large test cases.
    def canCompleteCircuit_bfs(self, gas: list[int], cost: list[int]) -> int:
        if sum(cost)>sum(gas): return -1
        que = deque()
        n = len(gas)
        for i in range(n):
            if cost[i]<=gas[i]:
                que.append(Node(gas[i], i, i))
        while len(que)>0:
            node = que.popleft()
            # print(node)0000
            if cost[node.cur]<=node.gas:
                next_i = (node.cur+1)%n
                if next_i==node.dest: return next_i
                node.gas = node.gas - cost[node.cur] + gas[next_i]
                node.cur = next_i
                que.append(node)
        return -1
