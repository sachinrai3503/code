...
class Solution:
    def longestStrChain(self, words: List[str]) -> int:
        ...
        words_chain_len = dict()
        for word in words:
            t_chain_len = 0
            word_len = len(word)
            for i in range(word_len):
                t_chain_len = max(t_chain_len, words_chain_len.get(word[:i] + word[i+1:], 0))
            max_len = max(max_len, t_chain_len+1)
            words_chain_len[word] = t_chain_len+1
        return max_len