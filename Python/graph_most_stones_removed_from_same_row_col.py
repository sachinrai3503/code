...
class DJSet:
    ...
    def union(self, i, j):
        ...
        if pi==pj: return
        rpi, rpj = self.rank[pi], self.rank[pj]
        if rpi>rpj:
            self.parent[pj] = pi
        elif rpi<rpj:
            self.parent[pi] = pj
...
class Solution:
    def removeStones(self, stones: List[List[int]]) -> int:
        x_dict = dict() # {i : parent of all nodes at x=i}
        y_dict = dict() # {j : parent of all nodes at y=j}
        n = len(stones)
        dj_set = DJSet(n)
        for i in range(n):
            x, y = stones[i]
            if x not in x_dict:
                ...
                dj_set.union(x_dict[x], i)
            if y not in y_dict:
                y_dict[y] = i
            else:
                dj_set.union(y_dict[y], i)
        root_count = 0
        # print(dj_set.parent)
        # print(dj_set.rank)
        for i in range(n):
