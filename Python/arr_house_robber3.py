...
class Solution:
    ...
    # def __init__(self):
    #     self.max_amount_robbed = 0

    def max_amount(self, root):
        if not root: return (0,0)
        left = self.max_amount(root.left)
        right = self.max_amount(root.right)
        with_root = root.val + left[1] + right[1]
        without_root = max(left) + max(right)
        # self.max_amount_robbed = max(with_root, without_root)
        return (with_root, without_root)

    def rob(self, root: Optional[TreeNode]) -> int:
        return max(self.max_amount(root))