...
class Solution:
    ...
    def knightDialer(self, n: int) -> int:
        if n==1: return 10
        MOD = 1000000007
        next_steps = {1:[6,8],2:[7,9],3:[4,8],4:[0,9,3],6:[1,7,0],7:[6,2],8:[1,3],9:[4,2],0:[4,6]}
        _next = [0 for i in range(10)]
        cur = [1 for i in range(10)]
        for k in range(2, n+1):
            for key in next_steps:
                steps = next_steps.get(key, [])
                for step in steps:
                    _next[step] = (_next[step] + cur[key])%MOD
            # print(_next)
            self.set_0(cur)
            cur, _next = _next, cur
        return sum(cur)%MOD
