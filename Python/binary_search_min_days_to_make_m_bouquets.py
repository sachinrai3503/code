...
class Solution:
    ...
    def count_bouquets_on_day(self, bloomDay, k, day):
        count = 0
        cur_bouquet_count = 0
        for bloom in bloomDay:
            if bloom<=day:
                cur_bouquet_count+=1
                if cur_bouquet_count==k:
                    count+=1
                    cur_bouquet_count = 0
            else:
                cur_bouquet_count = 0
    ...
    def minDays(self, bloomDay: List[int], m: int, k: int) -> int:
        bloomDay_len = len(bloomDay)
        if (m*k)>bloomDay_len: return -1
        op = -1
        s, e = min(bloomDay), max(bloomDay)
        while s<=e:
            mid = s + (e-s)//2
            if self.count_bouquets_on_day(bloomDay, k, mid)>=m:
                op = mid
                e = mid-1
            else:
                s = mid+1
