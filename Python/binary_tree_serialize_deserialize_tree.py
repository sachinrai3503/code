...
class Codec:
    ...
    def serialize_root(self, root, op):
        if root is None: op.append('N')
        else:
            op.append(str(root.val))
            self.serialize_root(root.left, op)
            self.serialize_root(root.right, op)
    ...
    def serialize(self, root):
        ...
        """
        op = list()
        self.serialize_root(root, op)
        # print(op)
        return '.'.join(op)

    def deserialize_root(self, tree:List[str]) -> TreeNode:
        if tree[self.i] == 'N':
            self.i+=1
            return None
        root = TreeNode(int(tree[self.i]))
        self.i+=1
        root.left = self.deserialize_root(tree)
        root.right = self.deserialize_root(tree)
        return root
    ...
    def deserialize(self, data):
        ...
        data = data.split('.')
        # print(f'{data=}')
        return self.deserialize_root(data)
