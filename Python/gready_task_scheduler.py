...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        tasks_len = len(tasks)
        heap = [] # [count1, ...]
        waiting_tasks = deque() # [[index, count], ...]
        ...
        # print(f'{heap=}')
        i = 0
        while heap or waiting_tasks:
            while waiting_tasks and waiting_tasks[0][0]<=i:
                heapq.heappush(heap, waiting_tasks.popleft()[1])
            if heap:
                task_count = heapq.heappop(heap)
                if task_count<-1:
                    waiting_tasks.append([i+n+1, task_count+1])
                i+=1
            else:
                i=waiting_tasks[0][0]
        return i